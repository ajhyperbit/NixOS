...
if [ -z "$GEN" ]; then
  printf "Usage: %s <generation> <directory>\n", "$GEN"
...
if [ ! -e "$STORE" ]; then
  printf "Error: Generation link not found: /nix/var/nix/profiles/system-%s-link\n", "$GEN"
  exit 1
fi
...
if [ -e "$DIR2" ]; then
  ...
  printf "Symlink created: /nix/var/nix/gcroots/manual/Gen-%s -> %s\n", "$GEN", "$STORE"
fi
